...
from pandas.core import strings as strs


def test_api(any_string_dtype):

    # GH 6106, GH 9322
    assert Series.str is strs.StringMethods
    assert isinstance(Series([""], dtype=any_string_dtype).str, strs.StringMethods)
...
def test_api_per_dtype(index_or_series, dtype, any_skipna_inferred_dtype):
    ...
    if inferred_dtype in types_passing_constructor:
        # GH 6106
        assert isinstance(t.str, strs.StringMethods)
    else:
...
def test_api_for_categorical(any_string_method, any_string_dtype, request):
    ...
    s = s + " " + s
    c = s.astype("category")
    assert isinstance(c.str, strs.StringMethods)
